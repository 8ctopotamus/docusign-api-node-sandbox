...
const createTabs = async contentHtml => {
  ...
  tabs = tabs.reduce((prev, curr) => {
    ...
    let y = Math.abs(Math.floor(coords.top) - (mt * 2))    
    const pageNumber = Math.ceil( y / pageHeight) 
    
    let internalPageHeight = pageHeight
    if (pageNumber % 2 === 0) {
      y -= mt 
    } 

    const difference = (internalPageHeight * pageNumber) - y 
    yPosition = Math.abs(internalPageHeight - difference)
